 ...
 * Attaches compliance metadata (consentVersion, dataResidency, processingBasis)
 * to the result for downstream saveScanResultWithCompliance calls.
 *
 * @example
...
export function useBloodReportAnalysis(
  options: UseBloodReportAnalysisOptions & {
    /** UID of the user running the analysis — used for audit logging */
    actorUid?:   string;
    /** Role of the user — used for audit logging */
    actorRole?:  'doctor' | 'patient';
    /** Override default compliance metadata for this session */
    compliance?: Partial<ComplianceMetadata>;
  },
): UseBloodReportAnalysisReturn & {
  /** Compliance metadata to attach when saving the result */
  ...
  const complianceMeta: ComplianceMetadata = {
    ...DEFAULT_COMPLIANCE,
    ...compliance,
  };
  ...
  const analyzeReport = useCallback(
    ...
    [groqApiKey, model, actorUid, actorRole, complianceMeta],
  );
  ...
  return { analyzeReport, isAnalyzing, result, error, reset, complianceMeta };
}